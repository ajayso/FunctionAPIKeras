...
class MultiInput:
    def load_data(self):
        ...
        df["sentiment_code"] = np.random.randint(0,1,len(df))
        ...
        sentiment = df["sentiment_code"]
        x_train,x_val,y_train,y_val,x_sentiment_train,embedding_matrix = self.Train_Embeddings(texts,labels,sentiment)
        x_train2,x_val2,y_train2,y_val2,x_sentiment_train,embedding_matrix2 = self.Train_Embeddings(description,labels,sentiment)
        print(len(x_train))
        print(len(x_train2))
        ...
        self.x_sentiment_train = x_sentiment_train
        
    
    def Train_Embeddings(self, texts,labels,sentiment):
        ...
        x_sentiment_train = sentiment[:training_samples]
        
        x_val = data[training_samples: training_samples + validation_samples]
        y_val = labels[training_samples: training_samples + validation_samples]
        x_sentiment_test = sentiment[training_samples: training_samples + validation_samples]
        ...
        return x_train,x_val,y_train,y_val,x_sentiment_train,embedding_matrix
    ...
    def CreateModel2(self):
        ...
        sentiment_prediction = layers.Dense(1, activation='sigmoid', name='sentiment')(concatenated)
        
        model = Model([text_input, question_input],[category_labels,sentiment_prediction])
        model.compile(optimizer='rmsprop', 
                      loss=['categorical_crossentropy','binary_crossentropy'], metrics=['acc'])
        model.summary()
        self.model = model
    ...
    def CreateModel(self):
        ...
        model = Model([text_input, question_input],[answers,sentiment_prediction])
    ...
    def ExecuteModel2(self):
        ...
        self.model.fit([self.train_data_text,self.train_data_descriptions], [self.train_data_labels,self.x_sentiment_train], epochs =100, batch_size =128)
